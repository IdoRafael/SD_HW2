  ...
  private static Injector setupAndGetInjector(String fileName) throws FileNotFoundException {
    ...
    Injector injector = Guice.createInjector(new BuyProductModule(), new AbstractModule() {
      ...
      protected void configure() {
        bind(FutureLineStorageFactory.class).to(FutureLineStorageFactoryTestImpl.class);
      }
    });
    BuyProductInitializer bpi = injector.getInstance(BuyProductInitializer.class);

//    (fileName.endsWith("xml") ? bpi.setupXml(fileContents) : bpi.setupJson(fileContents));
